...
fac.in.dat <- read.csv('LL_factors.csv', header = TRUE)
port.dat <- read.delim('LL_portfolios.txt', header = FALSE) #read.csv('25q_LL_portfolios.csv', header = TRUE

port_names <- c('s1b1','s1b2','s1b3','s1b4','s1b5','s2b1','s2b2','s2b3','s2b4','s2b5','s3b1','s3b2','s3b3','s3b4','s3b5','s4b1','s4b2','s4b3','s4b4','s4b5','s5b1','s5b2','s5b3','s5b4','s5b5')

names(port.dat) <- port_names 

fac.in.dat$cay.scaled <- fac.in.dat$cay -mean(fac.in.dat$cay[1:140])
fac.in.dat$cay.lag <- lag(fac.in.dat$cay.scaled, 1) 

fac.in.dat$c.int <- fac.in.dat$cay.lag * fac.in.dat$cons

fac.dat <- as.data.frame(cbind(fac.in.dat$Date, fac.in.dat$cons, fac.in.dat$cay.lag, fac.in.dat$c.int,fac.in.dat$RF))
names(fac.dat) <- c('Date','cons','cay','int','RF')


fin.dat <- cbind(port.dat, fac.dat[,2:(ncol(fac.dat)-1)])
fin.dat <- na.omit(fin.dat)
fac.dat <- na.omit(fac.dat)
...
retex.dat <- cbind((fin.dat[,1:ncol(port.dat)]), fin.dat[,(ncol(port.dat)+1):ncol(fin.dat)]) # cbind((fin.dat[,2:ncol(port.dat)] - fin.dat$RF), fin.dat[,(ncol(port.dat)+1):ncol(fin.dat)]) ### Get excess returns 
fac <- as.matrix(fac.dat[,2:(K+1)]) 


P1 <- as.matrix(fac[,1:K])   ### I am using the LL 3 factors 
...
R <- as.matrix(retex.dat[,1:N]) ### Convert to percentage 
...
q.conf.int <- matrix(c(0.00,0.39),2,1)
...
capm.out <- cbind(matrix(round(lambda.capm,4),(K+1),1),matrix(round(lambda.tstat,4),(K+1),1),matrix(c(round(capm.Rsq,2),'-','-','-'),(K+1),1),matrix(c(round(capm.gls.Rsq,2),'-','-','-'),(K+1),1),c(capm.tsq.out,'-','-'), c(q.out,'-','-'))

colnames(capm.out) <- c('Coefficients', 't-stat','Rsq OLS', 'Rsq GLS','Tsq Stat (p-val)', 'q (confidence interval)')
rownames(capm.out) <- c('Const.', 'consn.','cay','cay*consn')
...
#######################################
ind.dat <- read.csv('30ind_q_LL.csv', header = TRUE)
N.ind <- ncol(port.dat) + ncol(ind.dat) - 1
test.dat <- cbind(port.dat, ind.dat[,2:ncol(ind.dat)])
fin.ind.dat <- cbind(test.dat[2:141,], fac.dat[,2:ncol(fac.dat)])
retex.ind.dat <- cbind(fin.ind.dat[,1:ncol(port.dat)], (fin.ind.dat[,(ncol(port.dat)+1):(ncol(port.dat) + ncol(ind.dat))] - fin.ind.dat$RF)/100, fin.ind.dat[,(ncol(port.dat) +ncol(ind.dat)+1):ncol(fin.ind.dat)]) ### Get excess returns 
...
plot(q.grid, tsq.ind.quantiles[,1], ylab = expression(T^2 ~ 'statistic'), xlab = expression(q), main = expression('FF 25 + 30 Ind. (Conf.Int. for'~T^2~ 'and q)'), type = 'l', xlim = c(min(q.grid),max(q.grid)), ylim = c(min(tsq.ind.quantiles),max(tsq.ind.quantiles)))
...
q.ind.conf.int <- matrix(c(3.83,5.40),2,1)
...
rownames(capm.ind.out) <- c('Const.', '$consn.$', 'cay','consn.*cay')
...
#### Unwanted code : used to verify and match LL 2001 results #### 
#fac.dat <- as.data.frame(cbind(fac.in.dat$Date, fac.in.dat$cons)) #, fac.in.dat$c.int,fac.in.dat$RF))
#names(fac.dat) <- c('Date','cons')
#K <- 1