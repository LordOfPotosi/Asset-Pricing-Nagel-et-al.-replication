...
fac.in.dat <- read.csv('LL_fac2.csv', header = TRUE)
names(fac.in.dat) <- c('Date','cons','cay','RF')
...
names(port.dat) <- c('Date', port_names) 

fac.in.dat$cay.scaled <- fac.in.dat$cay -mean(fac.in.dat$cay[1:(nrow(fac.in.dat)-1)])
...
fin.dat <- cbind(port.dat, fac.dat)
...
R <- as.matrix(retex.dat[,1:N])/100 ### Convert to percentage 
...
q.conf.int <- matrix(c(0.00,0.20),2,1)
...
R.ind <- as.matrix(retex.ind.dat[,1:N.ind])/100
...
q.ind.conf.int <- matrix(c(0.49,0.00),2,1)
