...
fac.dat <- read.csv('ccapm_fac_current.csv', header = TRUE)
...
q.conf.int <- matrix(c(0.16,0.41),2,1)
...
capm.out <- cbind(matrix(round(lambda.capm,4),(K+1),1),matrix(round(lambda.tstat,2),(K+1),1),matrix(c(round(capm.Rsq,2),'-'),(K+1),1),matrix(c(round(capm.gls.Rsq,2),'-'),(K+1),1),c(capm.tsq.out,'-'), c(q.out,'-'))

colnames(capm.out) <- c('Coefficients', 't-stat','Rsq OLS', 'Rsq GLS','Tsq Stat (p-val)', 'q (confidence interval)')
rownames(capm.out) <- c('Const.', 'consn.')
...
plot(q.grid, tsq.ind.quantiles[,1], ylab = expression(T^2 ~ 'statistic'), xlab = expression(q), main = expression('FF 25 + 30 Ind. (Conf.Int. for'~T^2~ 'and q)'), type = 'l', xlim = c(0,max(q.grid)), ylim = c(min(tsq.ind.quantiles),max(tsq.ind.quantiles)))
...
q.ind.conf.int <- matrix(c(0.37,0.72),2,1)
...
capm.ind.out <- cbind(matrix(round(lambda.ind.capm,4),(K+1),1),matrix(round(lambda.ind.tstat,2),(K+1),1),matrix(c(round(capm.ind.Rsq,2),'-','-','-'),(K+1),1),matrix(c(round(capm.ind.gls.Rsq,2),'-','-','-'),(K+1),1),matrix(c(capm.ind.tsq.out,'-','-'),(K+1),1), matrix(c(q.ind.out,'-','-'),(K+1),1))
colnames(capm.ind.out) <- c('Coefficients ', 't-stat','R^2 OLS', '#Rsq GLS','Tsq Stat (p-val)', 'q (confidence interval)')
rownames(capm.ind.out) <- c('Const.', 'consn.')
