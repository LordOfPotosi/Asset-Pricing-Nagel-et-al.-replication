...
q.conf.int <- matrix(c(0.17,0.42),2,1)
...
capm.out <- cbind(matrix(round(lambda.capm,2),(K+1),1),matrix(round(lambda.tstat,2),(K+1),1),matrix(c(round(capm.Rsq,2),'-'),(K+1),1),matrix(c(round(capm.gls.Rsq,2),'-'),(K+1),1),capm.tsq.out, q.out)

colnames(capm.out) <- c('Coefficients', 't-stat','Rsq OLS', 'Rsq GLS','Tsq Stat (p-val)', 'q (confidence interval)')
rownames(capm.out) <- c('Const.', 'Rm')

xtable(capm.out, type = "latex", file = "tab1_capm_a.tex")
...
e.ind.gls <- mu.ind.R - C.ind.t %*% lambda.ind.gls.capm
capm.ind.gls.Rsq <- 1 - var(e.ind.gls)/var(mu.ind.R)
...
q.ind.conf.int <- matrix(c(0.39,0.76),2,1)
...
capm.ind.out <- cbind(matrix(round(lambda.ind.capm,2),(K+1),1),matrix(round(lambda.ind.tstat,2),(K+1),1),matrix(c(round(capm.ind.Rsq,2),'-'),(K+1),1),matrix(c(round(capm.ind.gls.Rsq,2),'-'),(K+1),1),capm.ind.tsq.out, q.ind.out)
colnames(capm.ind.out) <- c('Coefficients ', 't-stat','Rsq OLS', 'Rsq GLS','Tsq Stat (p-val)', 'q (confidence interval)')
rownames(capm.ind.out) <- c('Const.', 'Rm')

xtable(capm.ind.out, type = "latex", file = "tab1_capm_b.tex")
...
# capm.Rsq.1 <- 1 - (t(mu.gls.R) %*% M.gls.c %*% mu.gls.R/(t(mu.gls.R - mean(mu.gls.R)) %*% (mu.gls.R - mean(mu.gls.R))))*N/(N-K)
