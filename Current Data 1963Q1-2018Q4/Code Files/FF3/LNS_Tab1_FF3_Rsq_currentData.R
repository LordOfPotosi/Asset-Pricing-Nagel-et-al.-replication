...
### Calcualting the maximal Sharpe ratio: Write the general code for K factors ###
...
P1.SR <- t(P1.mean) %*% solve(P1.var) %*% P1.mean  ### This is used generally for computation of the GRS statistic (Not used here) 

#### First stage regressions to get C : R = a + P C + e ####

P <- cbind(matrix(1,T,1),P1)   ### Factor matrix for time series regressions

R <- as.matrix(retex.dat[,1:N]) ### Dependent variable for time series regressions 

C1 <- matrix(solve(t(P) %*% P) %*% t(P) %*% R,(K+1),N) ### Time series betas 
C <- matrix(C1[2:(K+1),],K,N)                          ### Cross section regressor matrix          

e <- R - P%*% C1                                
Sigma.e <- (t(e) %*% e)/T                              ### Time-series error variance (conditional variance of returns)
...
rsq.conf.int <- matrix(c(0.54,0.97),2,1)
...
rsq.conf.gls.int <- matrix(c(0.03,0.60),2,1)
...
rsq.conf.ind.int <- matrix(c(0.04,0.83),2,1)
...
rsq.conf.ind.gls.int <- matrix(c(0.56,0.76),2,1)
